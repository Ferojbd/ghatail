      ...
      <section class="md:w-full lg:w-full xl:w-full">
         <div class="bg-gray-800 pt-3">
                <div class=" bg-gradient-to-r from-blue-900 to-gray-800 p-4 shadow text-2xl text-white">
                    <h3 class="font-bold pl-2">EDIT ROLE</h3>
                </div>
            </div>
       <div class="p-5 m-5 bg-white text-gray-700 shadow-lg">
            <div class="flex">
               <div class="w-2/4">
                   <h1 class="font-bold text-2xl">EDIT ROLE</h1>
               </div>
            ...
            @if ($role)
                <form action="{{route('edit-role-action')}}" method="POST">
                @csrf
                    <div class="sm:w-full md:w-full lg:w-1/2 xl:w-1/2 m-2">
                        <input type="hidden" name="id" id="id" value="{{$role->id}}">
                        <label for="role" class="block">Role</label>
                        <input type="text" required class="w-full py-2 mt-2 border-2 border-gray-200 rounded px-2" value="{{$role->role}}" name="role" id="role" placeholder="Enter role">
